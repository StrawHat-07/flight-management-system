...
@Slf4j
public class DirectFlightPricingStrategy implements PricingStrategy {
    ...
    public BigDecimal calculatePrice(String flightIdentifier, int seats) {
        return flightServiceClient.getFlightById(flightIdentifier)
                .map(flight -> flight.getPrice().multiply(BigDecimal.valueOf(seats)))
                .orElseGet(() -> {
                    log.warn("Flight not found: {}", flightIdentifier);
